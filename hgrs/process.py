...
odir = '/sat_data/satellite/acix-iii/results/'
...
water_pixel_prop = (prod.coarse_masked_raster.water_pixel_number/prod.Npix_per_megapix).drop_vars('tcwv').rename({"x": "xc", "y": "yc"})
...
keys = ['wl_water_vapor', 'wl_sunglint', 'wl_atmo', 'wl_to_remove', 'wl_green', 'wl_nir', 'wl_1600', 'wl_rgb',
        ...
        'ang_resol', 'dirdata', 'abs_gas_file', 'lut_file', 'water_vapor_transmittance_file', 'sunglint_threshold',
        'ndwi_threshold', 'green_swir_index_threshold', 'pressure', 'to3c', 'tno2c', 'tch4c', 'psl', 'coef_abs_scat',
        'altitude']
for key in keys:
    l2_prod.attrs[key] = str(prod.__dict__[key])

encoding = {'Rrs': {'dtype': 'int16', 'scale_factor': 0.00001, '_FillValue': -9999},
            'aot_ref_full': {'dtype': 'int16', 'scale_factor': 0.001, '_FillValue': -9999},
            'aot_ref': {'dtype': 'int16', 'scale_factor': 0.001, '_FillValue': -9999},
            'aot_ref_std': {'dtype': 'int16', 'scale_factor': 0.001, '_FillValue': -9999},
            'brdfg_full': {'dtype': 'int16', 'scale_factor': 0.00001, '_FillValue': -9999},
            'brdfg': {'dtype': 'int16', 'scale_factor': 0.00001, '_FillValue': -9999},
            'brdfg_std': {'dtype': 'int16', 'scale_factor': 0.00001, '_FillValue': -9999},
            'tcwv_full': {'dtype': 'int16', 'scale_factor': 0.01, '_FillValue': -9999},
            'tcwv': {'dtype': 'int16', 'scale_factor': 0.01, '_FillValue': -9999},
            'tcwv_std': {'dtype': 'int16', 'scale_factor': 0.01, '_FillValue': -9999}}

ofile = odir + site + l1c.replace('.he5', '.nc').replace('L1_STD_OFFL', 'L2A_hGRS')
ofile = 'truc.nc'
