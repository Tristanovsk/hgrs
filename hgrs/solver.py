...
class WaterVapor(Solver):
    ...
    def get_wv_transmittance_raster(self):
        ...
        # TODO (Antoine): is fill_value behaviour ok (was the old beaviour))
        t_r = (
            self.Twv_lut.interp(air_mass=self.air_mass_mean)
            .interp(tcwv=self.water_vapor.tcwv)
            .drop("air_mass")
            .drop("tcwv")
        )
        ...
        fill_value = (
            self.Twv_lut.interp(air_mass=self.air_mass_mean)
            .interp(tcwv=np.nanmax(self.water_vapor.tcwv))
            .drop("air_mass")
            .drop("tcwv")
        )
...
class Aerosol(Solver):
    ...
    def prepare_lut(self, sensor_desc):
        """
        Used for:
        ...
        aot_lut = self.aero_lut.sel(model=self.aerosol_model).aot
        aot_lut = sensor_desc.sensor_mod_lr.convolve(aot_lut)
        self.aot_lut = aot_lut.interp(aot_ref=aot_refs, method="quadratic").dropna(
            "aot_ref"
        )

        norm_radiance = (
            self.aero_lut.sel(model=self.aerosol_model)
    ...
    def get_aot_diff_trans(self, wl):
        ...
        aots = self.aot_lut.interp(aot_ref=aot_ref_vals, method="linear")
        Rdiffs = self.Rtoa_lut.interp(aot_ref=aot_ref_vals, method="linear")
    ...
    def compute_atmo_bidir_transmittance(self):
        ...
        Ttot_Ed_ = sensor_mod.convolve(Ttot_Ed_)

        Ttot_Lu_ = Ttot_Ed_opacmodel.interp(sza=self.vza_mean, method="cubic").interp(
            aot_ref=aot_ref, method="quadratic"
        )
        Ttot_Lu_ = sensor_mod.convolve(Ttot_Lu_)
