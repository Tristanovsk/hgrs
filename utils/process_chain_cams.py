...
lut_file = '/data/lut/opac_osoaa_lut_v3.nc'
aero_lut = xr.open_dataset(lut_file)
aero_lut['wl']=aero_lut['wl']*1000

Ttot_Ed=xr.open_dataset('/data/lut/transmittance_downward_irradiance.nc')
...
for site in sites:
    ...
    for l1c_path in glob.glob(opj(workdir, '*L1_STD_OFFL*.he5')):
        ...
        Ttot_Ed_ = Ttot_Ed.Ttot_Ed.sel(model=opac_model).interp(sza=sza, method='cubic'
                                ).interp(aot_ref=aot_ref, method='quadratic').interp(wl=wl, method='cubic')
        Ttot_Lu_ = Ttot_Ed.Ttot_Ed.sel(model=opac_model).interp(sza=vza, method='cubic'
                                ).interp(aot_ref=aot_ref, method='quadratic').interp(wl=wl, method='cubic') ** 1.05
